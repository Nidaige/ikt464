...
for path in data_paths:
    ...
    if len(all_data[0]) == 0:
        ...
        all_data[1]+=lab
        all_data[2]+=labraw
X_all_data, Y_all_data, labels_all_data = shuffle_dataset(all_data)  # X_all_data: list of lists of features | y_all_data: 0/1/2/... , but wrong. Fixing here. | labels_all_data: string of the class of each object
labels_all_data_set = list(set(labels_all_data))  # create set of ALL string-labels
for i in range(len(Y_all_data)):  # for each element
    Y_all_data[i] = labels_all_data_set.index(labels_all_data[i])  # assign the true index to each data label Y
Y_all_data = np.array(Y_all_data)
labels_all_data = np.array(labels_all_data)
    ...
    for t_ in T:
        ...
        for test_data_sample in range(len(X_test)):
            ...
            if Prediction[test_data_sample] == Y_test[test_data_sample]:
                Correct += 1
                if Prediction[test_data_sample] == "BENIGN":
                    TP += 1
                else:
                    TN += 1
            else:
                if Prediction[test_data_sample] == "BENIGN":
                    ...
                    FN += 1
            '''
            pred = Prediction[test_data_sample]
            if pred == Y_test[test_data_sample] and labels_test[test_data_sample] == "BENIGN":
                benign_index = Y_test[test_data_sample]
                
            if pred == benign_index and pred == Y_test[test_data_sample]:  # if guessing benign and is correct:    
                Correct += 1
            elif pred != benign_index and Y_test[test_data_sample] != benign_index:  # elif guessing malign, and correct answer is malign:
                Correct += 1
            '''
                
        print("Accuracy: ", 100*Correct/Total , "%")
        path = "results_new_data_"+str(Clauses)+"_S_"+str(s_)+"_T_"+str(t_)+".txt"
        file = open(path,"w")
        file.write("Results: \n")
        ...
        file.write("FP: "+str(100*FP/Total)+"%, FN: "+str(100*FN/Total)+"%, TP: "+str(100*TP/Total)+"%, TN: "+str(100*TN/Total)+"% \n")
        file.close()
